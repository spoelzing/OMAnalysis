...
action = varargin{1};
global Log Contour Steve Stripchart Data Autocalc
switch action
    %================================================================
    case 'Initial'
        Steve.Applied=0;
   ...
   % Show Polar Output --------------------------------------------------
          Steve.CheckBox.Polar = uicontrol('Parent',Steve.Figure, ...
            'BackgroundColor',Contour.Color,...
            'Enable','off',...
            'FontName','Arial',...
            'FontUnits','points',...
            ...
            'FontAngle','normal',...
            'Units','pixel',...
            'Position', [510 52  118 20], ...
            'String','Show Polar Plot', ...
            'Value',0, ...
            'Style','checkbox');     
      
         Steve.Button.AllAngle=	uicontrol('Parent',Steve.Figure, ...
            'Units','pixels',...
            'Enable','off',...
            'FontUnits','points',...
            'FontWeight','normal',...
            'FontAngle','normal',...
            'Position',[440 50 60 30],...
            'Style','togglebutton',...
            'String','All Angles', ...
            'callback','Steve_Contour(''All_Angles'');', ...
            'Value',0,...
            'Tag','Pushbutton1');
         Steve.Text.Dilate=	uicontrol('Parent',Steve.Figure, ...
            'BackgroundColor',[0.8 0.8 0.8], ...
            'Units','pixels',...
            'Enable','off',...
            'FontUnits','points',...
            'FontWeight','normal',...
            'FontAngle','normal',...
            'Position',[440 30 60 15],...
            'Style','Text',...
            'String','Dilation');
         Steve.Edit.AllAngleText=	uicontrol('Parent',Steve.Figure, ...
            'Units','pixels',...
            'Enable','off',...
            'FontUnits','points',...
            'FontWeight','normal',...
            'FontAngle','normal',...
            'Position',[504 30 40 15],...
            'Style','edit',...
            'String','10');
         Steve.Text.AngleStep=	uicontrol('Parent',Steve.Figure, ...
            'BackgroundColor',[0.8 0.8 0.8], ...
            'Units','pixels',...
            'Enable','off',...
            'FontUnits','points',...
            'FontWeight','normal',...
            'FontAngle','normal',...
            'Position',[440 10 60 15],...
            'Style','Text',...
            'String','Step Angle');
         Steve.Edit.AngleStep=	uicontrol('Parent',Steve.Figure, ...
            'Units','pixels',...
            'Enable','off',...
            'FontUnits','points',...
            'FontWeight','normal',...
            'FontAngle','normal',...
            'Position',[504 10 40 15],...
            'Style','edit',...
            'String','5');    
        ...
        Steve.Button.Finish=	uicontrol('Parent',Steve.Figure, ...
            ...
            'Tag','Pushbutton1');
        % THE LINE IN THE BOX
        Steve.Frame.Main = uicontrol('Parent',Steve.Figure, ...
            'BackgroundColor',[0.8 0.8 0.8], ...
            'enable','off',...
            'ListboxTop',0, ...
            'Units','pixel',...
    ...
    case 'Mouse Down'
        ...
        if get(Steve.Button.Start,'Value')==1 & q(1,1)>.4 & q(1,1)<Steve.Xlim(2) & q(1,2)>.4 & q(1,2)<Steve.Ylim(2)
            Steve.Set=0;
            ...
            Steve.MarkerA=patch(...
                ...
                'YData',[XY(2)]);
            %SPCODEX
                 set([Steve.CheckBox.Polar, Steve.Button.AllAngle, Steve.Text.Dilate, Steve.Text.AngleStep, Steve.Edit.AllAngleText, Steve.Edit.AllAngleText, Steve.Edit.AngleStep],'Enable','on');  
        elseif get(Steve.Button.Finish,'Value')==1 & q(1,1)>0.4 & q(1,1)<Steve.Xlim(2) & q(1,2)>.4 & q(1,2)<Steve.Ylim(2)
            Steve.Set=0;
            Contour=get(Steve.Figure,'userdata');
            Steve.currentpoint=get(Steve.Axes,'currentpoint');
            Steve.currentpoint=(Steve.currentpoint(1,1:2));
        ...
        if ~isempty(findobj(Steve.Axes,'type','patch','tag','A')) & ~isempty(findobj(Steve.Axes,'type','patch','tag','B')) & Steve.Set==0;
            ...
            disp('******************************************')
            disp('CV - Mean, Median, Stdev, Members, Angle- Mean, Anatomic')
            [Steve.Vector.NewMagnitude; Steve.Vector.MagMed; Steve.Vector.MagStd; Steve.Members; Steve.Vector.NewAngle; Steve.LineAngle ]
            disp('******************************************')
            %Steve_Contour('Apply')
        end
    ...
    case 'All_Angles'
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % DILATION COMMANDS SPCODEX 2023
        % if get(Steve.Button.Start,'Value')==1 & q(1,1)>.4 & q(1,1)<Steve.Xlim(2) & q(1,2)>.4 & q(1,2)<Steve.Ylim(2)
        startpoint=[round(get(Steve.MarkerA,'YData')),round(get(Steve.MarkerA,'XData'))];
        if get(Steve.Button.Start,'Value')==1 & startpoint(1)>.4 & startpoint(1)<Steve.Xlim(2) & startpoint(2)>.4 & startpoint<Steve.Ylim(2)
            DilExt=str2num(get(Steve.Edit.AllAngleText,'String'))*2+1;
            AAA=sqrt(Contour.Vector.Vx.^2+Contour.Vector.Vy.^2);
            DDD=0*ones(size(Contour.Image));
            BBB=DDD;
            for arow=1:length(Contour.Vector.Y)
                DDD(Contour.Vector.Y(arow),Contour.Vector.X(arow))=AAA(arow);
            end
            %         clc
            %         figure(5)
            %         subplot(2,2,1),imagesc(DDD)
            BBB(find(DDD==0))=1;
            BBB=im2bw(BBB);
            %         subplot(2,2,2), imagesc(BBB)
            
            CCC=imdilate(BBB,ones(3));
            %          CCC=BBB;
            %         subplot(2,2,3), imagesc(CCC)
            s = regionprops(CCC);
            centroids = cat(1,s.Centroid);
            dist2point=((centroids(:,1)-startpoint(2)).^2+(centroids(:,2)-startpoint(1)).^2).^.5;
            [Y,ClosestCentroid]=min(dist2point);
            a=round(centroids(ClosestCentroid,1));
            b=round(centroids(ClosestCentroid,2));
            StimSite = imdilate(bwselect(CCC,a,b),ones(DilExt))-bwselect(CCC,a,b);
            Autocalc.StimSite=StimSite;
            
            aa=size(Contour.Image);
            %         imagesc(Autocalc.StimSite)
            i=1;
            %         figure
            %         subplot(1,2,1)
            %         if get(Contour.Check.Inverse,'Value')
            ...
            for col=1:aa(1)
                for row=1:aa(2)
                    if i<length(Contour.Vector.X) & Contour.Vector.X(i)==row & Contour.Vector.Y(i)==col
                        Contour.Vector.Vx(i)=Contour.Vector.Vx(i)*StimSite(col,row);
                        Contour.Vector.Vy(i)=Contour.Vector.Vy(i)*StimSite(col,row);
                        i=i+1;
                    end
                end
            end
            ...
            %         subplot(2,1,1)
            %         Vector=quiver(Contour.Vector.X,Contour.Vector.Y,Contour.Vector.Vx,Contour.Vector.Vy);
            %         subplot(2,1,2), imagesc(imerode(Autocalc.StimSite,ones(3)))
            Steve_Contour('Plot')
            
        end
        % END OF THE DILATION ALGORITHM
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        
        
%         Autocalc_Contour('Initial')
        Autocalc_Contour('All Angles')
    ...
    case 'DeleteFcn'
        if ~isempty(Steve.Figure)
            ...
            Temp=get(Steve.Figure);
            SteveContParams.FGSize=Temp.Position;
