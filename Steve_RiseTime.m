function [Label,Comment,Annote_Out]=Steve_RiseTime(Z,X1X2,ChLabel,Annote) 
global Log
global Data
...
Parameter5Description=('Takeoff Threshold (Percentage. ie. 5% of upstroke)');
Parameter6Description=('Peak Threshold (Percentage. ie. 90% of upstroke)');
P1Default=3;
P2Default=5;
P3Default=50;
P4Default=-0.7;
...
contin=RTInterface(Parameter1Description,Parameter2Description,Parameter3Description,Parameter4Description,Parameter5Description,Parameter6Description,PDefaults,Label);
...
if AT~=0
   ...
   Amp=max(DataCh2);
   EndRT=find(DataCh2(minindex:end)>=Log.Defaults.Parameter6/100*Amp)+minindex-1;
if ~isempty(EndRT) & EndRT(1)<ydim
      temp=fliplr(DataCh2);
      temp2=find(temp(ydim-EndRT(1):end)<=(Log.Defaults.Parameter5)/100*Amp);
      temp3=length(temp(ydim-EndRT(1):end));
      StartRT=temp3-temp2+2;
     
      temp4=find(temp(ydim-EndRT(1):end)<=(Log.Defaults.Parameter6)/100*Amp);
      temp5=length(temp(ydim-EndRT(1):end));
      EndRT=temp5-temp4+2;     
