...
if Log.Defaults.ParP7>=Log.Head.SRate/2-0.02*2/Log.Head.SRate;
   Log.Defaults.ParP7=Log.Head.SRate/2-(0.0200001/2*Log.Head.SRate);
...
for i=1:(length(Fig.ChLabel(:,1))-1)
  ...
  datadiff=diff(Fig.Data(channel,:));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     if length(fiducialX)>count & (fiducialX(count)+blockout)<length(DataCh)+Fig.X1X2(1)-10 & fiducialX(count)-Window>0 & ~isempty(find(datadiff(fiducialX(count)-offset:fiducialX(count))>=0))
        if fiducialX(count)+Window<fiducialX(count+1) & fidx1(count)~=0 & fidx1(count)-Window>0 % 0
     		DataCh=Log.Defaults.Parameter4/-abs(Log.Defaults.Parameter4)*(DataCh-min(DataCh(fidx1(count)-Window:fidx1(count)+Window)));
      	offset=round(Log.Defaults.ParP5/2);
         baseline=find(datadiff(fiducialX(count)-offset:fiducialX(count))>=0);
         baseline=baseline+fidx1(count)-offset;
        if get(Fig.Med,'Value')  % 1
	      bv=Fig.Data(channel,[baseline(end)-round(Log.Defaults.ParP5/2)]:baseline(end)-1);
	      [bv1 bv1i]=sort(bv);
	      medianbaseline=bv1i(round(length(bv1i)/2));
         medianbaselinevalue=DataCh(baseline(end)-medianbaseline);
        else
         [medianbaselinevalue,medianbaseline]=min(DataCh([baseline(end)-round(Log.Defaults.ParP5/2)]:baseline(end)-1));
        end     % end 1
         ...
         if length(Repol)>=1; % 2
			  NewAnnote1=[NewAnnote1;Fig.ChLabel(i,1) Fig.X1X2(1)+baseline(end)-medianbaseline 'Bas'];  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	elseif fidx1(count)-Window>0 & fidx1(count)+Window<length(DataCh) & fidx1(count)-Window>0 & ~isempty(find(datadiff(fiducialX(count)-offset:fiducialX(count))>=0))
      DataCh=Log.Defaults.Parameter4/-abs(Log.Defaults.Parameter4)*(DataCh-min(DataCh(fidx1(count)-Window:fidx1(count)+Window)));
      baseline=find(datadiff(fiducialX(count)-offset:fiducialX(count))>=0);
      baseline=baseline+fidx1(count)-offset;
 ...
 if 2==1;
  ...
  line([fidx1(count) fidx1(count)],[max(DataCh) min(DataCh)]);
...
if ~isempty(aChannelBaseline) & ~isempty(bChannelMaximums) & ~isempty(ChannelB) & count<=length(aChannelBaseline) & count<=length(bChannelMaximums)
   BaselineValue=DataCh(Baselines(aChannelBaseline(count),2)-Fig.X1X2(1));
   MaximumValue=DataCh(Maxs(bChannelMaximums(count),2)-Fig.X1X2(1)-1);
   Amplitude=abs(BaselineValue);
   % This filters the data for repolarization
   if Log.Defaults.ParP7<Log.Head.SRate/2-50
      DataCh=filter(B,1,DataCh);
   end
   Repol=find(DataCh(ChannelB(count)-Fig.X1X2(1):length(DataCh))>=Log.Defaults.ParP6/100*Amplitude)-1;
   if ~isempty(Repol)
      NewAnnote3=[NewAnnote3;i Repol(1)+ChannelB(count) Label];
   end
