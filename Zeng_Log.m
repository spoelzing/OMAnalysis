...
switch action
    ...
    case 'Initial'
        ...
        if Continue
            ...
            FG_Size(1:2)=[Log.UD.ScreenSize(1)-FG_Size(3)-35 Log.UD.ScreenSize(2)-FG_Size(4)-40 ];
            ...
            Log.Button.Delete = uicontrol('Parent',Log.Figure, ...
                ...
                'String','Remove', ...
                'callback','Zeng_Log(''Delete'',''Selected'')',...
                'visible','off',...          'enable','off',...
                'Tag','Pushbutton1');
            
            Log.Text.FileSz=[10 (FG_Size(4)-30) 65 22];
    ...
    case 'Read'
        ...
        if max([FileName Path]) ~= 0%if you do not click cancel
            ...
            Temp.SmLetters=find(FileName>=65 &  FileName<=90);
            FileName(Temp.SmLetters)=FileName(Temp.SmLetters)+32;
            ...
            if FID > -1 & ~isstr(FID);           %If .h exists.
                ...
                Log.Head.Help.PleaseRemoveTheFirst4Bits=0;
                if isempty(find(Head_Data==0));
                    ...
                    while feof(FID)==0
                        Line=fgets(FID);
                        if Line==-1 % Code added 01/04/13 By Sharon George for getting around New Ultima Header reading problem.
                            return  % Code added 01/04/13 By Sharon George for getting around New Ultima Header reading problem.
                        end         % Code added 01/04/13 By Sharon George for getting around New Ultima Header reading problem.
                    ...
                    if ~isfield(Log.Head,'Sys_Ver')
                        ...
                        if strcmp(Log.Head.Sys_Ver,lower('Zeng''s Extra cellular Mapping'))
                            Log.Head.Help.DataType=2;
                            Log.Head.RawDataVer=1;
                        elseif strcmp(Log.Head.Sys_Ver,lower('Optical Mapping System'))
                            Log.Head.Help.DataType=1;
                            Log.Head.RawDataVer=0;
                        end
                    end
                ...
                elseif length(Head_Data)==480 %         if isempty(find(Head_Data==0));
                    if Head_Data(91)==0;               %Ver 1st data.
                        ...
                        Log.Head.HeadRaw=Head_Data; %for save raw in Stripchart when you want to cut segment
                        ...
                        Log.Head.Sys_Ver ='CWRU Extra cellular';
                        
                        %Log.Head.LUTExt='tab';
                        Log.Head.Help.DataType=2;
                        Log.Head.RawDataVer=1;
                        
                        Log.Head.Platform='pc';
                        ...
                        Log.Head.Help.DataType=2;
                        Log.Head.Help.PleaseRemoveTheFirst4Bits=1;
                        Log.Head.Platform='pc';
    ...
    case 'Read Raw'
        ...
        elseif ~isempty(findobj('name',[Log.Head.FileName ':Stripchart']));
            ...
            if FID > -1 & ~isstr(FID) % if raw data exits.
                ...
                if 0 %ftell(FID)>5000000
                    ...
                    %  if ~sum(Format_Check-[3 67 79 76 85 77 78])==0  This
                    %  was commented out 5/10/16 because of errors on line
                    %  636
                    if 1 % The MiCAM cameras do not appear to require lines 626-656, hence I just commented out the above line and forced this to go in
                        % Not MIT file
                        if Log.Head.RawDataVer==0; %Optical mapping system format
                            FID=fopen([Log.Head.Path Log.Head.FileName],'r','b'); %big-endian
                            Temp=Stripchart.Figure.Number;
                            clear Stripchart.Figure
                            Stripchart.Figure=Temp;
                            Channel=Log.Head.Chans;
                            ...
                            if Log.Head.Help.PleaseRemoveTheFirst4Bits;
     ...
     %----------------------DELETE BELOW----------------------------------
                        ...
                        % The Two commented out lines below have been returning an
                        % error when the signal is not baseline corrected upon
                        % conversion for whatever reason. As a result, they have been
                        % commented out so that the header (of fixed length) is
                        % stripped from the data.
                        Head=fgetl(FID);
                        Head=[Head fgetl(FID)];
                        disp('Data header often has errors. Read note on lines 622-626 in Zeng_Log if error occurs')
                        if 1
                            Data{Stripchart.Figure}=zeros(Log.Head.Chans,Log.Head.Samples);
                            if 1
                                for i=1:Log.Head.Samples
                                    temp=fread(FID,Log.Head.Chans,'int16')';
                                    % The line below fails usually because one data point is missing. temp is
                                    % missing one data point at i=Log.Head.Samples.
                                    Data{Stripchart.Figure}(:,i)=temp;
                                end
                            else
                                Data{Stripchart.Figure}=fread(FID,[Log.Head.Chans,Log.Head.Samples],'int16')';
                            end
    ...
    case 'Delete'
        ...
        if Current_File>0 & ~isempty(Log.Object)
            ...
            Zeng_Error('There are no files in the log window');
        end
    ...
    case 'Display'
        global Log
        set(Log.Button.Display,'enable','off')
        %Log=get(gcbf,'userdata');
        ...
        set(Log.Button.Display,'enable','on')
