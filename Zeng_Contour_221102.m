            ...
            if 0%isempty(Contour.WaveForm)
                ...
                if isempty(Contour.Ch_XY)
                    ...
                    if ~isempty(a)
                        ...
                        contourfigure.FGSize=[200 200 800 500];
                        contourfigure.colormap=flip(cbrewer('seq','YlGnBu',64));
                        contourfigure.label=1;
                        Contour.Interval=3;
                        contourfigure.interval=3;
                    end
                    ...
                    Label2={ %Please set the tag to 'vargin{3}'
                        'Label'              ' '                                        ' '
                        ...
                        '>None'              'Zeng_Contour(''Label'',gcbf,''None'')     ' 'None'};
                    Contour.Menu.Label=makemenu(Contour.Figure,char(Label2(:,1)),char(Label2(:,2)), char(Label2(:,3)));
%                     if contourfigure.label==1;
%                         set(Contour.Menu.Label(contourfigure.label),'Checked','on');
%                         set(Contour.Menu.Label(contourfigure.label),'tag','Ch');
%                     end


                    TProcess={ %Please set the tag to  equal to 'vargin{3}'
                        'Process'                  ' '                                            ' '
                        '>Average Filter'          'Zeng_Contour(''Filter'',gcbf,''Mean'')        ' '1'
                        '>Median Filter'           'Zeng_Contour(''Filter'',gcbf,''Median'')      ' '2'
                        '>Nearest Neighbor'        'Zeng_Contour(''Filter'',gcbf,''Near'')         ' '3'
                        '>Auto Vector'             'Autocalc_Contour(''Initial'')                 ' '4'};
                    ...
                    %-------------------------
                    %To plot image
                    Zeng_Contour('Label',Contour.Figure,contourfigure.label)
        ...
        if isnumeric(Type)
            ...
            if Type=='BuGnYl'
                BuGnYl=flip(cbrewer('seq','YlGnBu',64));
            end
            eval(['colormap(' Type ')']);
        end
        ...
        switch action
            ...
            case 'ClrMap'
                ...
                if ~isempty(ClrMap)
                    ...
                    eval(['load','([''',[Log.UD.Path Log.UD.CD,'Mat', Log.UD.CD, 'ctable'],'''])']);
                    if Type=='BuGnYl'
                        BuGnYl=flip(cbrewer('seq','YlGnBu',64));%sets default colormap as BuGnYl (flipped YlGnBu) by GB 20210105
                    end
                    eval(['colormap(' Type ')']);
                    if isfile('contourfigure.mat')
                        load contourfigure
                    end
                    load contourfigure
    ...
    case 'Default'
        %nargin=2, you reset it
        %nargin=3, you change Label
        Contour=get(varargin{2},'userdata');
        Stripchart=get(Contour.Parent,'userdata');
        Continue=0;
        ...
        if isempty(Stripchart.Annote.Array)
            ...
            elseif strcmp(Contour.Analysis,'Difference')
                ...
                if isempty(Contour.XYT)
                    ...
                    Contour.DTime=[min(Temp.sort(:,1)) max(Temp.sort(:,2))];
                    Continue=1;
                end
            else
        ...
        load contourfigure
        contourfigure.interval=Contour.Interval;
        save contourfigure contourfigure
    ...
    case 'CntrlVar'
        ...
        switch Label
            case 'Mapping Array'
                ...
                if isfile('contourfigure.mat')
                    if ~isempty(contourfigure.label)
            ...
            case 'Difference'
                ...
                if Continue
                    ...
                    set(Contour.Menu.ContourType,'enable','on');
                    set(Contour.Menu.Label,'enable','on');
                    set(Contour.Edit.Interval,'string',num2str(Contour.Interval));
                end
            otherwise
...
%                     set([Contour.Menu.ContourType Contour.Menu.Label],'enable','on');
    ...
    case 'Label'
        ...
        if nargin ==2
            ...
            if contourfigure.label==1;
                set(findobj(Contour.Menu.Label,'tag','None'),'Checked','on');
                set(Contour.Menu.Label(contourfigure.label),'tag','Ch');
...
%                     end
                if ~isempty(contourfigure.label)
                    text(Xlim(1)+(Contour.Ch_XY(:,2)-Contour.Xlim(1)-0.25)*ScaleX, Ylim(1)+(Contour.Ch_XY(:,3)-Contour.Ylim(1))*ScaleY, num2str(Contour.Ch_XY(:,1)),'tag','Label','fontsize',Log.UD.Ref.FontSize-2)
                end
                case 'Time'
                    if get(Contour.Check.Offset,'value')
                        Offset=str2num(get(Contour.Edit.Offset,'string'));
                    ...
                    for i = 1:length(Contour.Ch_XY(:,1))
                        ...
                        text(Xlim(1)+(Contour.Ch_XY(i,2)-Contour.Xlim(1)-0.25)*ScaleX, Ylim(1)+(Contour.Ch_XY(i,3)-Contour.Ylim(1))*ScaleY,num2str(min(T(t2))),'tag','Label','fontsize',Log.UD.Ref.FontSize-2)
                ...
                case 'Ch+Time'
                    ...
                    for i=1:length(Contour.Ch_XY(:,1))
                        text(Xlim(1)+(Contour.Ch_XY(i,2)-Contour.Xlim(1)-0.25)*ScaleX,Ylim(1)+(Contour.Ch_XY(i,3)-Contour.Ylim(1)-0.2)*ScaleY,num2str(Contour.Ch_XY(i,1)),'tag','Label','fontsize',Log.UD.Ref.FontSize-2)
                        t2=find(Contour.XYT(:,1)==Contour.Ch_XY(i,2) & Contour.XYT(:,2)==Contour.Ch_XY(i,3) );
                        text(Xlim(1)+(Contour.Ch_XY(i,2)-Contour.Xlim(1)-0.25)*ScaleX,Ylim(1)+(Contour.Ch_XY(i,3)-Contour.Ylim(1)+0.2)*ScaleY,num2str(min(T(t2))),'tag','Label','fontsize',Log.UD.Ref.FontSize-2)
                    end
                case 'None'
            end
        end
        contourfigure.label=action;
        save contourfigure contourfigure
...
%-----------------------------------------------------------------------
    case 'Plot TimeMap'
        ...
        Contour=get(varargin{2},'userdata');
        Continue=0;
        if strcmp(Contour.Analysis,'Difference')
            Temp.Tmax=Contour.DTime(2);
            Temp.Tmin=Contour.DTime(1);
        else
            ...
            if isempty(Stripchart.Annote.Array)
                ...
                if isempty(Temp.Index)
                    ...
                    if strcmp(Contour.Analysis,'Difference')
                        Temp.sort=[];
                        for i=1:length(Contour.Ch_XY)
                            Temp.Ch=find(Stripchart.Annote.Array(Temp.Index,1)==Contour.Ch_XY(i,1));
                            if length(Temp.Ch)>1 %at least 2
                                %it is also in the LUT
                        ...
                        %Contour.Time from Diff and unDiff are different
                    ...
                    if isempty(Contour.XYT)
                        ...
                        switch varargin{4}
                            ...
                            case 'Vectors'
                                ...
                                %use Contour.ImageHD for being a background when you click it and want to change the background color
                        ...
                        if Continue
                            axes(Contour.Axes)
                            if strcmp(Contour.Analysis,'Difference')
                                caxis([round(str2num(get(Contour.Edit.Tmin,'string'))) round(str2num(get(Contour.Edit.Tmax,'string')))]);
                            else
                                %                 caxis([Contour.Time]); 211202
                                caxis([round(str2num(get(Contour.Edit.Tmin,'string'))) round(str2num(get(Contour.Edit.Tmax,'string')))]);
                            end
    ...
    case 'Offset'
        ...
        axes(Contour.Axes);
        ClrLength=size(get(Contour.Figure,'colormap'),1);
        %Get the position of the YTick
        % Bar=get(Contour.HDColorbar,'YTick')*diff(Contour.Time)/(ClrLength-1)
        %'Position',[4 158 413 270]);
        %Temp=findobj(Contour.Axes,'tag','Label')
        %if ~isempty(Temp)
        ...
        if strcmp(Contour.Analysis,'Difference')
            Ylim=[round(str2num(get(Contour.Edit.Tmin,'string'))) round(str2num(get(Contour.Edit.Tmax,'string')))];
        else
            Ylim=Contour.Time;
        end
%         Ylim=get(Contour.HDColorbar,'ylim');
        YTick=get(Contour.HDColorbar,'YTick');

        Offset=str2num(get(Contour.Edit.Offset,'string'));

        if 1==1;%strcmp(Contour.CurrentPlot,'Image') || (strcmp(Contour.CurrentPlot,'Contour') && get(Contour.Check.Color,'value'))
...
% 111
                ...
                if Offset>=0
%%%%%%%%%%%%%%%%%%%%NEEDS HELP WHEN OFFSET IS CHECKED
% clc
% 222
                    Ylabel=(Ylim(1):round(diff(Ylim)/6):Ylim(2))-Offset;
%                     YLimits=(Ylim-Offset);
                    %                     Ylabel=YTick-Offset;
                else
                    Zeng_Error(['Please add a positive number for the offset' char(13) 'OK !!!'])
...
% %%%%%%%%%%%%%%%%%%%%NEEDS HELP WHEN OFFSET IS NOT CHECKED
% clc
% 333

%                     Ylabel=round(YTick/(ClrLength)*(diff(Contour.Time))+Contour.Time(1)-Offset)
                    Ylabel=(Ylim(1):round(diff(Ylim)/6):Ylim(2));
                    YLimits=Ylim;
%                 Ylabel=YTick
            end

%                set(Contour.HDColorbar,'location','eastoutside','Ticks',round(linspace(0,Ylabel(end),6)),'TickLabels',Ylabel)%,'Limits',YLimits) % ensures colorbar is to right of contour plot
               set(Contour.HDColorbar,'location','eastoutside','TickLabels',Ylabel)%,'Limits',YLimits) % ensures colorbar is to right of contour plot
...
%  set(Contour.HDColorbar,'unit','pixel','position',[Temp.F(3)-50 Temp.A(2) 20 Temp.A(4)],'ytick',YTick,'YTickLabel',Ylabel); 211202
Ylabel
            set(Contour.HDColorbar,'unit','pixel','position',[Temp.F(3)-50 Temp.A(2) 20 Temp.A(4)],'YTick',Ylabel);
        end
...
% % % % % % % % % % % % % % % % %         YLabel2=Ylabel
    ...
    case 'Mouse Down'
        ...
        if get(Contour.Button.Quadrant,'Value')==1 && get(Contour.Button.Quadrant,'Enable')=='on'
            load junk;
...
%                    Contour.VP
      ...
      case 'Filter'
          ...
          %         Contour=get(gcbf,'userdata');
          action=get(gcbo,'Tag');
          Tmin=str2num(get(Contour.Edit.Tmin,'String'));
          switch action
              case '1'
                  bsteve=[];
                  TempImage=wthresh(Contour.Image,'h',Tmin);
                  TempImage(isnan(TempImage))=0;
                  TempImage(find(~TempImage))=Tmin;
                  bsteve=conv2(TempImage,1/9*ones(3),'same');
              case '2'
                  bsteve=[];
                  TempImage=wthresh(Contour.Image,'h',Tmin);
                  TempImage(isnan(TempImage))=0;                  
                  TempImage(find(~TempImage))=Tmin;
                  bsteve=medfilt2(TempImage,[3 3],'symmetric');
              case '3'
                  bsteve=[];
                  TempImage=wthresh(Contour.Image,'h',Tmin);
                  TempImage(isnan(TempImage))=0;
                  for row=1:size(TempImage,1)
                      zeropos=TempImage(row,:)==0;
                      if any(zeropos)
                          zeropos=find(zeropos);
                          non_zero=1:size(TempImage,2);non_zero(zeropos)=[];
                          ...
                          TempImage(row,zeropos)=(TempImage(row,idx(1,:))+TempImage(row,idx(2,:)))/2;
                          bsteve=medfilt2(TempImage,[3 3],'symmetric');
%                           bsteve=TempImage;
                      end
                  end
    ...
    case 'Quadrant' %Added by Poelzing October 2021
        ...
        Temp.Tmax=round(str2num(get(Contour.Edit.Tmax,'string')));
        Temp.Tmin=round(str2num(get(Contour.Edit.Tmin,'string')));
        bsteve(find(bsteve<Temp.Tmin))=NaN;
        bsteve(find(bsteve>Temp.Tmax))=NaN;
        ...
        set(Contour.Text.Mean,'String',['Mean = ',num2str(mean(qall))])
        disp('')
        disp(['Stdev APD '])
        set(Contour.Text.Std,'String',['Std = ',num2str(std(qall))])
        disp(num2str(std(qall)));

        set(Contour.Text.Min,'String',['Min = ',num2str(min(qall))])
        set(Contour.Text.Max,'String',['max = ',num2str(max(qall))])
    ...
    case 'ArrAr'
        ...
        elseif varargin{2}=='E'
            ...
            if Arithm==1
                ...
                Data=(ArrayA.data)-(ArrayB.data);
                %imagesc(Data)

                [row,col]=size(ArrayA.data);
                valsA=reshape(ArrayA.data,row*col,1);
                valsB=reshape(ArrayB.data,row*col,1);
                ...
                figure(11)
                plot(valsA, valsB,'c*') %plot individual points

                title(['Correlation Between ', ArrayA.Name ,' and ', ArrayB.Name] );
                %get fitted values
